...
# Defaults may be overridden by persisted CONFIG saved on disk
...
# Where to persist data (Render disk will be mounted at /data)
...
# Prices (can be updated via admin commands and persisted)
PRICE_CONFIG = {
...
# Runtime in-memory state (will be saved/loaded)
PENDING_PAYMENTS: Dict[str, Dict[str, Any]] = {}
PURCHASE_LOG: list = []
KNOWN_USERS: set = set()
SENT_INVITES: dict = {}
CONFIG: dict = {}  # persisted config (channels, payment info, price overrides)
...
# Admin commands (brief — same behavior as earlier)
...
def main():
    ...
    # join request handler for verify-on-join
    ...
    t = threading.Thread(target=_autosave_thread, daemon=True)
    t.start()
